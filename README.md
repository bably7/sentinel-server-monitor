...
- 使用自定义背景图与液态玻璃视觉风格
- 支持选择本机 JPG 或 PNG 图片作为自定义背景，并同步到桌面浮窗
...
点击主面板右上角的背景按钮，可以选择本机 JPG 或 PNG 图片。应用会检查文件和像素尺寸、自动校正手机照片方向，并将图片缩放到合理尺寸后保存到当前用户的 Sentinel 数据目录；透明 PNG 会保留透明通道，原图片不会被修改。新背景会立即同步到主面板和桌面浮窗，也可以在同一设置窗口中恢复内置默认背景。
...
- 新增主面板自定义背景图片功能，支持 JPG 和 PNG
- 自定义背景实时同步到桌面浮窗并在重启后保留
- 支持一键恢复内置默认背景
- 自动校正 JPEG 的 EXIF 拍摄方向，并保留透明 PNG 的透明通道
- 增加文件大小、像素尺寸、解码结果和输出大小校验
- 串行化背景读取、选择与恢复操作，避免并发写入和启动竞态
- 增加 PNG、JPEG 和 EXIF 元数据解析测试
